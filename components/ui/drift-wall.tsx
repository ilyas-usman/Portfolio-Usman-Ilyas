...
export function DriftWall({
    ...
    const handleResize = () => {
      if (typeof window !== 'undefined') {
        if (window.innerWidth < 640) {
          setEffectiveColumns(Math.min(3, columns));
        } else if (window.innerWidth < 1024) {
          setEffectiveColumns(Math.min(4, columns));
        } else {
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className="drift-wall__inner" style={item.bgColor ? { backgroundColor: item.bgColor } : undefined}>
        {item.image && (
        ...
        {item.title && (
          <span className="drift-wall__content">
            {item.category && (
              <span className="text-[10px] font-mono uppercase tracking-wider text-red-400 bg-black/60 px-2 py-0.5 rounded border border-white/10 self-start">
                {item.category}
              </span>
            )}
            <div>
              <span className="text-sm font-black tracking-tight drop-shadow-md text-white block">
                {item.title}
              </span>
              {item.tag && (
                <span className="text-[10px] font-mono text-zinc-300 block opacity-90">
                  {item.tag}
                </span>
              )}
            </div>
    ...
    const commonProps = {
      ...
      onBlur: release,
      onPointerDown: () => activate(id, colIndex),
      onTouchStart: (e: React.TouchEvent) => {
        e.stopPropagation();
        if (activeIdRef.current === id && !item.href) {
          release();
        } else {
  ...
  return (
    <div
      ...
      onTouchStart={(e: React.TouchEvent<HTMLDivElement>) => {
        ...
        if (target && !target.closest('[data-tile-id]')) {
          release();
        }
      }}
